...
interface FilterBarProps {
  ...
  onThemeChange: (subCategories: string[]) => void;
...
export const FilterBar = ({
  ...
  // Read initial subcategory filters from URL
  const initialSubCategories = searchParams
    .get("sub_categories")
    ?.split(",") || [];
  ...
  const toggleSubCategory = (subCat: string) => {
    ...
    if (subCat === "all") {
      ...
      newSelection = selectedSubCategories.includes(subCat)
    ...
    // Update URL params (comma-separated)
    const params = new URLSearchParams(searchParams);
    if (newSelection.length) {
      params.set("sub_categories", newSelection.join(","));
    } else {
  ...
  return (
    <div className="py-6 sticky top-20 z-40 glass">
      <div className="section-container space-y-4 relative">
        ...
        {/* THEME / SUBCATEGORY PILLS */}
        <motion.div
          className={`${
            showFilters ? "flex flex-wrap" : "hidden"
          } lg:flex lg:flex-nowrap lg:overflow-x-auto lg:no-scrollbar flex-wrap gap-2 pb-1`}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        >
          <button
            onClick={() => toggleSubCategory("all")}
          ...
          >
            ...
            <svg viewBox="0 -3 103 40" preserveAspectRatio="none">
              <rect x="1.8" y="2.5" width="99" height="28" rx="20" ry="20" pathLength="0.9" />
            </svg>
          </button>
          ...
          {themes.map((theme) => (
            <button
              key={theme._id}
              onClick={() => toggleSubCategory(theme.name)}
              className={`theme-border-anim px-4 py-[9px] flex items-center justify-center rounded-full text-sm font-medium whitespace-nowrap transition-all duration-300 bg-transparent relative overflow-hidden ${
                selectedSubCategories.includes(theme.name)
                  ? "active text-primary"
                  : "text-muted-foreground hover:text-primary"
              }`}
            >
